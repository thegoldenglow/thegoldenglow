...
const TaskManagement = () => {
  ...
  const handleTaskFormSubmit = async (e) => {
    ...
    const taskData = {
      ...
      link: taskFormData.targetUsername || null // Add link field to database update
    };
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-deepLapisDark arabesque-border p-6 rounded-lg shadow-xl w-full max-w-lg my-8">
            ...
            <form onSubmit={handleTaskFormSubmit} className="space-y-4">
              ...
              {taskFormData.taskCategory === 'Content Engagement' && (
                <div className="space-y-4">
                  <div>
                    <label htmlFor="postUrl" className="block text-sm font-medium text-textLight/70 mb-1">Social Media Post URL</label>
                    <input
                      type="url"
                      name="postUrl"
                      id="postUrl"
                      value={taskFormData.postUrl}
                      onChange={handleTaskFormChange}
                      className="w-full bg-deepLapis/50 border border-royalGold/30 rounded-md p-2 text-textLight focus:ring-royalGold focus:border-royalGold"
                      placeholder="e.g., https://youtube.com/watch?v=... or https://instagram.com/p/..."
                      required
                    />
                  </div>
                  <div>
                    <label htmlFor="requiredViewingTime" className="block text-sm font-medium text-textLight/70 mb-1">Required Viewing Time (seconds)</label>
                    <input
                      type="number"
                      name="requiredViewingTime"
                      id="requiredViewingTime"
                      value={taskFormData.requiredViewingTime}
                      onChange={handleTaskFormChange}
                      min="5"
                      max="300"
                      className="w-full bg-deepLapis/50 border border-royalGold/30 rounded-md p-2 text-textLight focus:ring-royalGold focus:border-royalGold"
                      placeholder="30"
                    />
                    <p className="text-xs text-textLight/50 mt-1">Users must view the embedded post for this duration to complete the task</p>
                  </div>
                </div>
              )}
