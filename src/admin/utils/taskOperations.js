    ...
    if (data && data.length > 0) {
      ...
      if (taskData.gameIdentifier) {
        ...
        try {
          ...
          console.warn('RPC postgrest_rpc failed; task created without game_identifier patch:', rpcErr?.message || rpcErr);
        }
      }
  ...
  try {
    ...
    if (taskData.gameIdentifier !== undefined) {
      ...
      try {
        ...
        console.warn('RPC postgrest_rpc failed; continuing without game_identifier patch:', rpcErr?.message || rpcErr);
      }
    }
