  ...
  try {
    ...
    if (isSupabaseAvailable()) {
      ...
      if (usersError) {
        ...
        // Don't throw, just return local users
      } else {
        dbUsers = data;
      }
      ...
      console.log('Supabase not available, loading only local users.');
    }
...
export const createRealUser = async (userData) => {
  try {
    if (!isSupabaseAvailable()) {
      return { user: null, error: 'Supabase is not available. Cannot create a real user.' };
    }
...
export const updateUserPoints = async (userId, points) => {
  try {
    ...
    if (userId.toString().startsWith('test_')) {
      ...
      if (!isSupabaseAvailable()) {
        return { success: false, error: 'Supabase is not available. Cannot update DB user points.' };
      }
