...
const ReferralLinkBox = () => {
  ...
  const completeSocialTaskIfNeeded = async () => {
    try {
      ...
      const socialTask = tasksState.tasks.find((t) => t.type === TaskType.SOCIAL && !t.completed);
      if (socialTask) {
        await tasksManager.completeTask(socialTask.id);
      }
    } catch (e) {
      console.warn('Failed to auto-complete SOCIAL task:', e?.message || e);
    }
  };
  ...
  return (
    <div className="bg-base-200 p-4 rounded-lg shadow-md">
      ...
      <div className="mt-4 flex gap-2">
        ...
        <button
           onClick={async () => {
             const shareText = 'Hey! Join me in The Golden Glow game. Use my referral link!';
             try {
