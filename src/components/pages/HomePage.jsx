...
const HomePage = () => {
  ...
  // Disable automatic redirects back to homepage
  useEffect(() => {
    // We want to stop any automatic redirects back to home page after the component is mounted
    return () => {
      // Cleanup function to prevent any redirects when component unmounts
    };
  }, []);
    ...
    <HomeLayout>
      {showIntro ? (
        ...
        <div className="w-full max-w-4xl mx-auto px-4 py-6">
          {/* User welcome */}
          <motion.div 
            className="mb-8 p-4 bg-deepLapisLight/30 rounded-lg border border-royalGold/20"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2">
              <h1 className="text-2xl font-primary text-royalGold mystic-glow">
                {user ? `Welcome, ${user.name || user.username || 'User'}` : <span className="mystic-decorative">Welcome, Traveler</span>}
              </h1>
          ...
          {user && (
            <motion.div
              className="relative z-20 mt-6 mb-4 space-y-3"
            ...
            >
              <Button
                ...
                className="relative z-20 pointer-events-auto flex items-center justify-center space-x-2"
              >
          ...
          {user && (
            ...
            >
              <div className="bg-deepLapisLight/95 p-3 rounded-lg">
