...
import { FiChevronLeft, FiAward, FiCreditCard, FiActivity, FiAlertCircle } from 'react-icons/fi';
...
const RewardsPage = () => {
  const { user, isLoading, updateUserFromLocalStorage } = useUser();
  ...
  useEffect(() => {
    // If user is null but we have a user in localStorage, try to load it
    if (!user && !isLoading && !isGuestMode) {
      ...
      if (localUser) {
        ...
        const result = updateUserFromLocalStorage();
        if (!result.success) {
          setError('Failed to load user data. Please try logging in again.');
        }
      }
    }
  }, [user, isLoading, updateUserFromLocalStorage]);
  
  // Show loading state
  if (isLoading) {
    return (
      <div className="text-center py-10">
        <p className="text-textLight mb-4">Loading rewards...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-center py-10">
        ...
        <p className="text-textLight mb-4">{error}</p>
  ...
  // Handle not logged in state, but allow guest mode
  if (!user && !isGuestMode) {
    return (
      <div className="text-center py-10">
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div>
          ...
          <span className="text-credits font-bold" data-component-name="RewardsPage">
            {user ? (user.points || 0) : localStorage.getItem('gg_anonymous_score') || 0} GC
          </span>
        </div>
      </div>
