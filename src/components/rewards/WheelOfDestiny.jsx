...
const WheelOfDestiny = () => {
  ...
  const handleSpin = useCallback(async (spinType) => {
    ...
    if (spinType === 'free' && !wheelOfDestiny.freeSpinAvailable) {
      ...
      alert(`You've already used your daily free spin! Come back in ${hoursUntilReset} hour${hoursUntilReset !== 1 ? 's' : ''} for your next free spin.`);
      return;
    }
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setTimeout(async () => {
          ...
          // Claim the reward using the stored spin ID
          if (result.segment.value > 0) {
            await handleClaimReward(currentSpinId, result.segment);
          } else {
            // For "Try Again" or zero-value segments
            await claimWheelReward(currentSpinId);
          }
  ...
  const handleClaimReward = useCallback(async (spinIdToClaim, segmentToClaim) => {
    if (!spinIdToClaim || !segmentToClaim) {
      console.error('Missing spin ID or segment in handleClaimReward', { spinIdToClaim, segmentToClaim });
      return;
    }

    console.log('Attempting to claim reward for spin:', spinIdToClaim, 'with segment:', segmentToClaim);

    try {
      // Call claimWheelReward and capture the result (now async)
      const claimResult = await claimWheelReward(spinIdToClaim);
      console.log('Claim wheel result:', claimResult);

      if (claimResult && claimResult.success) {
        // Handle successful claim
        if (claimResult.amount > 0) {
          // Add transaction for positive amounts
          addTransaction(
            claimResult.amount,
            'wheel_of_destiny',
          ...
          alert(`Congratulations! You won ${claimResult.amount} Golden Credits!`);
          console.log(`Successfully claimed ${claimResult.amount} Golden Credits from Wheel of Destiny`);
        } else {
          console.log('Claimed a zero-value reward');
        }
      } else if (claimResult) {
        // Handle unsuccessful claim with message
        console.warn('Claim result not successful:', claimResult.message);
        alert(`Unable to claim reward: ${claimResult.message}`);
      } else {
        // Handle missing result
        console.warn('No claim result returned from claimWheelReward');
        alert('Something went wrong while claiming your reward. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    // Check if we're in guest mode or have an authenticated user
    const isGuestMode = localStorage.getItem('gg_guest_mode') === 'true';
    ...
    // If in guest mode, make sure we have default wheel state
    if (isGuestMode && (!wheelOfDestiny || !wheelOfDestiny.segments || wheelOfDestiny.segments.length === 0)) {
      console.log('WheelOfDestiny: Creating default wheel state for guest mode');
      // This would be handled by your context, this is just a safety check
    }
        ...
        {selectedSegment && !isSpinning && (
          ...
          >
            <p className="text-xl font-primary text-ancientGold-light">
              Landed on: <span className="font-bold" data-component-name="WheelOfDestiny">{selectedSegment.label}</span>
            </p>
