...
export const GameProvider = ({ children }) => {
  ...
  const loadGames = useCallback(async () => {
    try {
      ...
      if (devModeEnabled) {
        // Load existing games from localStorage to preserve stats
        const savedGames = localStorage.getItem('gg_games');
        let gamesToUse;
        
        if (savedGames) {
          // Parse existing games and unlock all while preserving stats
          const existingGames = JSON.parse(savedGames);
          gamesToUse = existingGames.map(game => ({
            ...game,
            unlocked: true // Set all games to unlocked for testing
          }));
        } else {
          // No saved games, use default data with all unlocked
          gamesToUse = gameData.map(game => ({
            ...game,
            unlocked: true // Set all games to unlocked for testing
          }));
        }
        ...
        localStorage.setItem('gg_games', JSON.stringify(gamesToUse));
        setGames(gamesToUse);
        console.log('DEV MODE: All games unlocked for testing, stats preserved');
      } else {
