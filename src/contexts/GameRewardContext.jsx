  ...
  const awardGameReward = useCallback((gameId, rewardType, params = {}) => {
    ...
    switch (gameId) {
      case 'marks-of-destiny':
        result = awardMarksOfDestinyReward(rewardType, params);
        break;
        
      case 'path-of-enlightenment':
        result = awardPathOfEnlightenmentReward(rewardType, params);
        break;
        
      case 'flame-of-wisdom':
        result = awardFlameOfWisdomReward(rewardType, params);
        break;
        
      case 'gates-of-knowledge':
        result = awardGatesOfKnowledgeReward(rewardType, params);
        break;
        
      case 'sacred-tapping':
        result = awardSacredTappingReward(rewardType, params);
        break;
        
      case 'mystical-tap-journey':
        result = awardMysticalTapJourneyReward(rewardType, params);
        break;
    ...
    // Broadcast an event so other parts of the app (e.g., Tasks) can react to game rewards
    try {
      if (typeof window !== 'undefined' && result && result > 0) {
        const detail = { gameId, rewardType, amount: result, params };
        window.dispatchEvent(new CustomEvent('gg:gameRewardAwarded', { detail }));
      }
    } catch (e) {
