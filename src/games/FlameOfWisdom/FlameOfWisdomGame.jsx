...
const FlameOfWisdomGame = () => {
  ...
  // Set up game intervals when game is started
  useEffect(() => {
    ...
    const pointsInterval = setInterval(() => {
      ...
      if (pointsTimer >= POINTS_INTERVAL) {
        ...
        // Save game session and update points every 10 seconds (every 200 intervals of 50ms)
        if (Math.floor(Date.now() / 1000) % 10 === 0) {
          saveGameScoreAndUpdatePoints({
            gameType: 'flame-of-wisdom',
            score: flameLevel,
            pointsEarned: pointsEarned,
            duration: Math.floor((Date.now() - (gameStarted ? Date.now() - sessionPoints * 100 : Date.now())) / 1000),
            completed: false, // Ongoing session
            gameData: {
              flameLevel,
              tapCount,
              sessionPoints,
            ...
            console.error('Failed to save flame game session:', error);
            // Fallback to old method
            updateUserPoints(pointsEarned);
          });
        } else {
          // For non-save intervals, just update points locally
          updateUserPoints(pointsEarned);
        }
    ...
    // Clean up intervals on unmount or when game stops
    return () => {
      clearInterval(decayInterval);
      clearInterval(pointsInterval);
    };
  }, [gameStarted]); // Only re-run when gameStarted changes
  ...
  const handleTap = async () => {
    ...
    // Note: Tap points (0.5 per tap) will be batched and saved with the periodic game sessions
    // We'll accumulate them locally and save periodically to avoid too many database calls
    setSessionPoints(prev => prev + 0.5);
    await updateUserPoints(0.5); // Wait for the points to update for immediate feedback
    ...
    setFlameSize(prevSize => {
      ...
      if (newLevel > flameLevel) {
        ...
        if (newLevel % 5 === 0 || newLevel === 10 || newLevel === 15 || newLevel === 20) {
          const achievementReward = awardGameReward('flame-of-wisdom', 'level', { level: newLevel });
  ...
  useEffect(() => {
    ...
    if (timerActive && timeRemaining > 0) {
      ...
      // Award tap milestone rewards for the session
      if (timerTapCount > 0) {
        const tapReward = awardGameReward('flame-of-wisdom', 'taps', { taps: timerTapCount });
        
        if (tapReward > 0) {
          setRewardNotification({
            amount: tapReward,
            source: `${timerTapCount} Taps Completed`
          });
  ...
  }, [timerActive, timeRemaining]); // Removed unnecessary dependencies that could cause timer restart
    ...
    >
      <div className="flex flex-col items-center justify-center w-full max-w-md mx-auto h-full">
        {!gameStarted ? (
          ...
          >
            {/* HUD */}
            <div className="w-full flex justify-between items-center mb-1 gap-2">
              <div className="py-1 px-2 sm:py-2 sm:px-4 bg-deepLapisLight/50 rounded-md flex-1 min-w-0">
                <p className="text-xs text-white/70">Flame Level</p>
                <p className="text-sm sm:text-lg text-royalGold truncate">{flameLevel}</p>
              </div>
              
              <div className="py-1 px-2 sm:py-2 sm:px-4 bg-deepLapisLight/50 rounded-md flex-1 min-w-0">
                <p className="text-xs text-white/70">Golden Credits</p>
                <p className="text-sm sm:text-lg text-white flex items-center">
                  <span className="truncate">{user?.points !== undefined ? parseFloat(user.points).toFixed(1) : '0.0'}</span>
                  {sessionPoints > 0 && (
                    <span className="text-xs text-royalGold ml-1 animate-pulse hidden sm:inline">(Session: +{sessionPoints})</span>
                  )}
                </p>
              </div>
            </div>
            ...
            <div className="w-full bg-deepLapisLight/50 rounded-md p-2 sm:p-3 mb-2">
              <div className="flex justify-between items-center mb-1">
                <p className="text-white/70 text-xs sm:text-sm">30 Second Challenge</p>
                <p className="text-royalGold font-medium text-sm">
              ...
              <div className="flex justify-between items-center gap-2">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm sm:text-lg text-white">{timerTapCount}</p>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-white/70">Record</p>
                  <p className="text-sm sm:text-lg text-royalGold">{timerHighScore}</p>
                </div>
                {!timerActive && (
                  <button 
                    onClick={restartTimer}
                    className="bg-royalGold/30 hover:bg-royalGold/50 text-white text-xs py-1 px-2 sm:px-3 rounded transition-colors flex-shrink-0"
                  >
                    Try Again
                  </button>
                )}
            ...
            {/* Flame container */}
            <div 
              className="relative w-full aspect-square max-w-[280px] sm:max-w-xs mx-auto rounded-full bg-gradient-to-b from-deepLapis/40 to-deepLapis/80 border border-royalGold/20 flex items-center justify-center cursor-pointer overflow-hidden mb-2 sm:mb-4"
              onClick={handleTap}
            >
              ...
              <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                {tapCount > 0 && (
                  <motion.div
                    ...
                    transition={{ duration: 0.5 }}
                    className="absolute w-16 h-16 sm:w-20 sm:h-20 bg-royalGold/20 rounded-full"
                  />
                )}
              </div>
              
              <div className="absolute bottom-2 sm:bottom-4 left-0 right-0 text-center text-white/50 text-xs sm:text-sm">
                Tap to nurture
              </div>
            </div>
            
            {/* Game controls */}
            <div className="flex flex-col w-full gap-2 sm:gap-3 mb-3 sm:mb-6">
              <Button 
                variant="primary" 
                fullWidth
            ...
            <div className="w-full p-2 sm:p-3 bg-deepLapisLight/30 rounded-lg">
              <p className="text-xs text-white/70 mb-1 sm:mb-2">Active Enhancements</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-1 sm:gap-2">
                {upgrades.filter(u => u.level > 0).map(upgrade => (
                  <div key={upgrade.id} className="text-xs bg-deepLapis/50 rounded p-1.5 sm:p-2">
                    <span className="text-royalGold text-xs">{upgrade.name}</span>
                    <span className="text-white/80 block text-xs">Level {upgrade.level}</span>
                  </div>
                ))}
                {upgrades.filter(u => u.level > 0).length === 0 && (
                  <p className="text-white/50 text-xs col-span-full">None active</p>
                )}
              </div>
            </div>
          </motion.div>
        ...
        {showUpgradeModal && (
          ...
          >
            <motion.div 
              className="bg-deepLapis border-2 border-royalGold rounded-lg max-w-md w-full p-3 sm:p-5 max-h-[90vh] overflow-y-auto"
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              transition={{ duration: 0.3 }}
            >
              <div className="flex justify-between items-center mb-3 sm:mb-4">
                <h3 className="text-lg sm:text-xl text-royalGold font-primary">Spiritual Upgrades</h3>
              ...
              <div className="space-y-2 sm:space-y-4 mb-3 sm:mb-5">
                {upgrades.map(upgrade => {
                  ...
                  return (
                    <div 
                      key={upgrade.id} 
                      className={`p-2 sm:p-3 rounded-lg border ${canAfford ? 'border-royalGold/50' : 'border-white/10'}`}
                    >
                      <div className="flex flex-col sm:flex-row sm:justify-between gap-2">
                        <div className="flex-1 min-w-0">
                          <h4 className="text-royalGold font-medium text-sm sm:text-base">{upgrade.name}</h4>
                          <p className="text-white/70 text-xs sm:text-sm">{upgrade.description}</p>
                        ...
                        <div className="flex-shrink-0">
                          ...
                          >
                            {cost} <Icon name="wisdom" size={10} color="currentColor" className="ml-1" />
                          </Button>
                        </div>
                      </div>
                    </div>
