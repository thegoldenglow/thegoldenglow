...
const MarksOfDestinyGame = () => {
  ...
  useEffect(() => {
    ...
    if (result) {
      ...
      // Increased delay to ensure the final move is visible before win state
      setTimeout(() => {
        try {
            ...
            try {
              if (winnerValue === playerSymbols.player1) {
                ...
                if (gameMode === 'ai') {
                  ...
                  // Save game score and update points
                  try {
                    console.log('Attempting to save game score with points:', legacyPoints);
                    const result = await saveGameScoreAndUpdatePoints({
                      gameType: 'marks-of-destiny',
                      score: legacyPoints,
                      pointsEarned: legacyPoints,
                      duration: Math.floor((Date.now() - gameStartTime) / 1000) || 60, // Estimate if no start time
                      completed: true,
                      gameData: {
                        gameMode,
                        aiDifficulty,
                    ...
                    console.log('Save game score result:', result);
                    
                    if (!result.success) {
                      console.error('Failed to save game score:', result.error);
                      console.log('Using fallback addPoints method with points:', legacyPoints);
                      // Fallback to old method
                      addPoints(legacyPoints);
                    } else {
                      console.log('Game score and points saved successfully!');
                    }
                  } catch (error) {
            ...
            >
              ...
              <div className="space-y-4 mb-6">
                ...
                <Button 
                  ...
                  onClick={() => navigate('/games/marks-of-destiny-multiplayer')}
                >
                  🌟 Cosmic Duel (Online)
                </Button>
              </div>
