...
const MysticalTapJourneyGame = () => {
  ...
  // Game timer and animation frame - use refs to prevent race conditions
  ...
  useEffect(() => {
    ...
    const gameLoop = (timestamp) => {
      ...
      if (!gameLoopActiveRef.current || gameOver) {
        return;
      }
      ...
      setDistance(prevDistance => {
        ...
        if (newDistance >= maxDistance) {
          // Stop game loop before completing journey
          gameLoopActiveRef.current = false;
          completeJourney();
          return maxDistance;
        }
  ...
  const completeJourney = useCallback(() => {
    ...
    // Save game score and update points using the proper function
    setTimeout(async () => {
      try {
        const result = await saveGameScoreAndUpdatePoints({
          gameType: 'mystical-tap-journey',
          score: maxDistance,
          pointsEarned: totalWisdom,
          duration: elapsedTime,
          completed: true,
          gameData: {
            tapCount,
            wisdomCollected,
            visitedCities: visitedCities.map(city => city.name),
            journeyMode,
            difficultyLevel,
            averageTapRate: tapCount > 0 ? (tapCount / Math.max(1, elapsedTime)) * 60 : 0
          }
        });
        
        if (!result.success) {
          console.error('Failed to save game score:', result.error);
          // Fallback to old method if saveGameScoreAndUpdatePoints fails
  ...
  }, [tapCount, elapsedTime, maxDistance, wisdomCollected, updateUserPoints, saveStats, gameData, visitedCities, journeyMode, difficultyLevel, saveProgress, awardGameReward]);
