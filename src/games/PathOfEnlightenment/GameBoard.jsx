...
const GameBoard = ({ gameOver, onGameOver, onScoreUpdate, onRestart }) => {
  ...
  useEffect(() => {
    const calculateSize = () => {
      ...
      // Use more conservative padding for mobile devices
      const isMobile = window.innerWidth <= 768;
      const horizontalPadding = isMobile ? 40 : 32; // Increased padding on mobile
      const verticalOffset = isMobile ? 280 : 250; // More space for mobile UI
      
      const viewportWidth = Math.min(window.innerWidth - horizontalPadding, 350);
      const viewportHeight = window.innerHeight - verticalOffset;
      const availableSize = Math.min(viewportWidth, viewportHeight);
      
      // Ensure minimum size but cap maximum for mobile
      const minSize = isMobile ? 260 : 280;
      const maxSize = isMobile ? Math.min(300, window.innerWidth - horizontalPadding) : 350;
      
      setBoardSize(Math.max(minSize, Math.min(availableSize, maxSize)));
    };
  ...
  // Use ref to prevent race conditions during animations
  ...
  const moveTiles = useCallback((direction) => {
    if (isAnimating || gameEnded || animationInProgressRef.current || !gameActiveRef.current) return;
  ...
  return (
    ...
    >
      <div 
        className="relative bg-deepLapis/50 p-2 rounded-lg border border-royalGold/30 mx-auto"
        style={{
          width: `${boardSize}px`,
          height: `${boardSize}px`,
          maxWidth: '100vw',
