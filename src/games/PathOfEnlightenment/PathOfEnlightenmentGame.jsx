...
import { saveGameScoreAndUpdatePoints, getUserGameStats } from '../../utils/gameScoreManager';

const PathOfEnlightenmentGame = () => {
  console.log('PathOfEnlightenmentGame component loaded');
  ...
  console.log('Component state - user:', user, 'loading:', loading, 'gameData:', gameData);
  ...
  // Handle game over
  const handleGameOver = async (finalScore, maxTile) => {
    setGameOver(true);
    ...
    // Calculate points earned (1 point per 100 score)
    const pointsEarned = Math.floor(finalScore / 100);
    ...
    try {
      const result = await saveGameScoreAndUpdatePoints({
        ...
        gameData: {
          ...
          bestTile: Math.max(maxTile, bestTile)
        }
      });
    ...
    if (finalScore > bestScore) {
      ...
      saveStats({ highestScore: finalScore });
      // Save to localStorage for non-authenticated users
      localStorage.setItem('path-of-enlightenment-best-score', finalScore.toString());
      console.log('Saved new best score to localStorage:', finalScore);
    }
    
    // Update best tile
    if (maxTile > bestTile) {
      setBestTile(maxTile);
      saveStats({ highestTile: maxTile });
      
      // Award golden points based on highest tile achieved (legacy system)
      if (maxTile >= 64) {
        const tilePoints = Math.floor(Math.log2(maxTile) * 5);
        // Remove duplicate updateUserPoints call - points are handled by saveGameScoreAndUpdatePoints
  ...
  const handleScoreUpdate = useCallback((newScore, newMaxTile) => {
    ...
    // Update best score in real-time if current score exceeds it
    if (newScore > bestScore) {
      setBestScore(newScore);
      saveStats({ highestScore: newScore });
      // Save to localStorage immediately for non-authenticated users
      localStorage.setItem('path-of-enlightenment-best-score', newScore.toString());
      console.log('Real-time best score saved to localStorage:', newScore);
    }
  ...
  // Load best score and tile from various sources
  useEffect(() => {
    const loadGameStats = async () => {
      if (loading) return;
      
      let loadedBestScore = 0;
      let loadedBestTile = 2;
      
      if (user) {
        // For authenticated users, try database first, then fallback to gameData
        try {
          console.log('Loading best score for user:', user.id);
          const gameStats = await getUserGameStats('path-of-enlightenment', user.id);
          console.log('Game stats from database:', gameStats);
          loadedBestScore = gameStats.bestScore || 0;
          
          // If database doesn't have score, try gameData
          if (loadedBestScore === 0 && gameData?.stats?.highestScore) {
            loadedBestScore = gameData.stats.highestScore;
          }
          
          // Load best tile from gameData if available
          if (gameData?.stats?.highestTile) {
            loadedBestTile = gameData.stats.highestTile;
          }
          
          console.log('Setting best score to:', loadedBestScore);
          setBestScore(loadedBestScore);
          setBestTile(loadedBestTile);
        } catch (error) {
          console.error('Error loading best score from database:', error);
          // Fallback to gameData if database fails
          if (gameData?.stats) {
            loadedBestScore = gameData.stats.highestScore || 0;
            loadedBestTile = gameData.stats.highestTile || 2;
            setBestScore(loadedBestScore);
            setBestTile(loadedBestTile);
          }
        }
      } else {
        // For non-authenticated users, use localStorage first, then gameData
        console.log('No user authenticated, checking local storage');
        const localBestScore = parseInt(localStorage.getItem('path-of-enlightenment-best-score') || '0', 10);
        console.log('Local best score from localStorage:', localBestScore);
        
        // Use localStorage value if it exists, otherwise use gameData
        if (localBestScore > 0) {
          loadedBestScore = localBestScore;
        } else if (gameData?.stats?.highestScore) {
          loadedBestScore = gameData.stats.highestScore;
        }
    ...
    >
      <div 
            ...
            <div className="mt-2 mb-auto mx-auto max-w-xs">
              ...
              <div className="flex justify-between">
                <div>
                  ...
                  <p className="text-lg text-white">{Math.max(score, bestScore)}</p>
                </div>
                <div>
        ...
        {(gameStarted || gameOver) && (
          ...
          >
            {/* Score display */}
            <div className="flex justify-center gap-4 mb-4 px-4 max-w-sm mx-auto">
              <div className="text-center py-2 px-3 bg-deepLapisLight/50 rounded-md flex-1 min-w-0">
                <p className="text-xs text-white/70">Score</p>
                <p className="text-lg text-royalGold font-mono">{score}</p>
              </div>
              
              <div className="text-center py-2 px-3 bg-deepLapisLight/50 rounded-md flex-1 min-w-0">
                <p className="text-xs text-white/70">Best</p>
                <p className="text-lg text-white font-mono">{Math.max(score, bestScore)}</p>
              </div>
            </div>
            ...
            {gameOver && (
              ...
              >
                ...
                <p className="text-white text-center mb-4">
                  {bestTile >= 2048 
                    ? 'You have reached the pinnacle of golden enlightenment with the 2048 tile!' 
                    : `You achieved a ${bestTile} tile on your path.`}
                </p>
                
                {pointsEarned > 0 && (
                  <div className="bg-royalGold/20 p-2 rounded text-center mb-2">
                    <p className="text-royalGold">+{pointsEarned} Golden Points Earned</p>
                  </div>
                )}
            ...
            >
              ...
              <div className="space-y-4 text-white/90 mb-5">
                ...
                <p>
                  ...
                  Create the 2048 tile to achieve spiritual enlightenment and unlock golden points.
                </p>
                <p>
