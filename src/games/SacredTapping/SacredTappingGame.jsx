...
const SacredTappingGame = () => {
  ...
  // Use refs to prevent race conditions with intervals
  ...
  const startGame = (selectedDifficulty) => {
    ...
    gameTimerRef.current = setInterval(() => {
      setGameTime(prevTime => {
        if (!gameActiveRef.current || prevTime <= 1) {
          // End game when timer reaches zero or game is no longer active
          if (spawnIntervalRef.current) {
            clearInterval(spawnIntervalRef.current);
            spawnIntervalRef.current = null;
          }
  ...
  const endGame = async () => {
    ...
    const pointsEarned = Math.floor((score / 100) * difficultyMultiplier);
    ...
    try {
      ...
      if (result.success) {
        ...
        // Update local state to reflect the new points
        if (result.data.pointsUpdate) {
          setPointsAwarded(result.data.pointsUpdate.pointsEarned);
        }
      } else {
    ...
    // Remove the duplicate updateUserPoints call since saveGameScoreAndUpdatePoints already handles it
    ...
    // Calculate wisdom points for stats tracking
    const wisdomPoints = Math.floor((score / 100) * difficultyMultiplier);
