...
export const getTelegramBotToken = async (forceRefresh = false) => {
  ...
  try {
    ...
    if (!cachedToken) {
      console.error('No Telegram bot token available. Please set VITE_TELEGRAM_BOT_TOKEN environment variable.');
    ...
    if (!DEFAULT_TOKEN) {
      console.error('No fallback Telegram bot token available. Please set VITE_TELEGRAM_BOT_TOKEN environment variable.');
      throw new Error('Telegram bot token not configured');
    }
