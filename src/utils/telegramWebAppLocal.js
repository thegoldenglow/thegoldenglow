...
function initializeTelegramWebApp() {
  ...
  let initData = params.get('initData') || '';
  
  // Only create mock user if explicit URL parameters are provided
  if (!userId || !userName) {
    console.log('[TelegramWebApp] No user parameters provided, not creating mock user');
    return null;
  }
  ...
  const initDataUnsafe = {
    ...
    user: {
      ...
      username: userName.toLowerCase().replace(/\s/g, '_') || `user_${userId}`,
  ...
  // Generate initData string from the user data if not provided
  if (!initData) {
    const userData = encodeURIComponent(JSON.stringify(initDataUnsafe.user));
    initData = `user=${userData}&auth_date=${initDataUnsafe.auth_date}&hash=mock_hash`;
  }
...
// Log creation details if LocalTelegramWebApp was created
if (LocalTelegramWebApp) {
  console.log('LocalTelegramWebApp created with initData:', LocalTelegramWebApp.initData);
  console.log('LocalTelegramWebApp created with user:', LocalTelegramWebApp.initDataUnsafe.user);
}

/**
 * Initialize WebApp with either official Telegram WebApp or local implementation
 * @returns {Object|null} The WebApp object or null if no user data available
 */
export function setupTelegramWebApp() {
  ...
  // Try to create local implementation
  const localWebApp = initializeTelegramWebApp();
  
  if (!localWebApp) {
    console.log('No Telegram WebApp available and no user parameters provided');
    // Don't create window.Telegram.WebApp if no user data
    return null;
  }
  
  // If local implementation created successfully, use it
  console.log('Using local Telegram WebApp implementation');
  window.Telegram = window.Telegram || {};
  window.Telegram.WebApp = localWebApp;
