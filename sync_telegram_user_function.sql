...
-- Create RLS policy for Telegram users
CREATE POLICY "Telegram users can access their profile" ON profiles
  FOR ALL USING (
    telegram_id IS NOT NULL AND 
    (
      auth.uid() = id OR 
      -- Allow access if the user is authenticated via Telegram
      current_setting('app.telegram_user_id', true) = telegram_id
    )
  );
